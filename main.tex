...
\affil[4]{Department of Archaeology, University of Cambridge, Cambridge, CB2 3QG, UK.}
...
Data on the prevalence of herpes simplex virus 2 between 2000 and 2015 CE was taken from the supplementary materials of~\citep{Looker2015}, and plotted to demonstrate the distribution of HSV2 across Africa (see supplementary~\cref{fig:hsv2}). 
...
\begin{table}[!tbhp]
\caption{Probable HSV2 transmission paths}
\centering
\renewcommand{\arraystretch}{1.5}
...
\bottomrule
\multicolumn{3}{l}{* The most probable path has a cost of 1. The path costs are normalised to the shortest path.}
\end{tabular}
\end{table}
...
\begin{table}[!h]
	\centering
	\caption{Summary of sensitivity analyses showing the frequency of occurrence of different transmission routes using both HSV2-IP and HSV2-IT infection models. In total 4.6 million analyses were performed for each model.}
	\label{table:paths}  
	\begin{tabulary}{\linewidth}{LCCCCCC}
		\toprule
		Model & \multicolumn{3}{c}{Primary transmission path} & \multicolumn{3}{c}{Second transmission path}\\
		\cmidrule{2-7}
		& Intermediary species & Path cost \textbf{(normalised)} & \% occurrence &
		Intermediary species & Path cost \textbf{(normalised)} & \% occurrence \\
		\midrule
		HSV2-IP & \textit{P. boisei} & 0.587 \textbf{(1.0)} & 100\% & - & - & - \\
		\specialcellleft{HSV2-IP\\ (no critical path)} & \textit{H. habilis} & 	\specialcell{0.634 - 0.775\\ \textbf{(1.08 - 1.32)}} & 67\% & \textit{H. rudolfensis} & \specialcell{0.56 - 0.775\\ \textbf{(1.11 - 1.32)}} & 33\%\\
		HSV2-IT & \textit{H. habilis} $\rightarrow$ \textit{P. boisei} & \specialcell{0.866 - 1.34\\ \textbf{(1.0 - 1.547)}} & 60\% & \textit{P. boisei} & 
		\specialcell{0.866 - 1.34\\ \textbf{(1.0 - 1.547)}} & 40\% \\
		\bottomrule
	\end{tabulary}
\end{table}
\end{landscape}
...
\begin{figure}[!h]
	...
	\caption{Map showing the prevalence of HSV2 in Africa, using data from Looker (2015). The locations of hominin fossils [supplementary table A.1] are shown with markers. The colour of the marker indicates the hominin genus; the symbol represents the species. 
