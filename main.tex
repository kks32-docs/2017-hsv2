...
\RequirePackage[backend=biber, style=authoryear, citestyle=authoryear, sorting=nty, natbib=true, doi=false,isbn=false,url=false]{biblatex}
...
(normalised path cost) \\ 
\midrule
anc-chimp $\Rightarrow$ \textit{boisei} $\Rightarrow$ \textit{erectus} & \textbf{1 (1.0)} & 3 (1.14)\\


anc-chimp $\Rightarrow$ \textit{habilis}  $\Rightarrow$ \textit{boisei} $\Rightarrow$ \textit{erectus} & 4 (1.13) & \textbf{1 (1.0)} \\

anc-chimp $\Rightarrow$ \textit{habilis} $\Rightarrow$ \textit{erectus} & 2 (1.20) & 2 (1.05) \\

anc-chimp $\Rightarrow$ \textit{rudolfensis} $\Rightarrow$ \textit{erectus} & 3 (1.24) & 4 (1.54) \\
